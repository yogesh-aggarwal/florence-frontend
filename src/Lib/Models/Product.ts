...
export namespace Product {
   ...
   export async function FetchByIDS(ids: string[]): Promise<Product_t[] | null> {
      const res = await new API().post<{ data: Product_t[] }>(`/product/getByIDS`, { ids: ids })
      if (!res) return null
   ...
   export async function FetchByCategory(category: string): Promise<Product_t[] | null> {
      const res = await new API().get<{ data: Product_t[] }>(`/product/category/${category}`)
      if (!res) return null
   ...
   export async function FetchBySearchQuery(query: string): Promise<Product_t[] | null> {
      const res = await new API().get<{ data: Product_t[] }>(`/product/search/${query}`)
      if (!res) return null
