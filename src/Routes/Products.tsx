...
import { ProductDescription_t, ProductReview_t, Product_t } from "../Lib/Types/product"
import { Cart } from "../Lib/cart"
import { useNetworkRequest } from "../Lib/helpers"
import { ReviewCard } from "../Components/ReviewCard"

function ProductInfo(props: { heading: string; content: ProductDescription_t[] }) {
   return (
...
export default function Product() {
   ...
   useNetworkRequest(
      ...
      async (res: Response) => {
         ...
         const doesExist = products.find((product) => product.id === selectedProduct.id)
         if (doesExist) return
         ...
         <div id="body">
            <div id="left">
               <div id="slider">
                  ...
                  <div className="icons">
                     <div
                        className={"icon " + (product.images.indexOf(url) === 0 ? "disabled" : "")}
                        onClick={() => {
                           const imagePos = product.images.indexOf(url)
                           if (imagePos === 0) return
                     ...
                     <div
                        className={
                           "icon " +
                           (product.images.indexOf(url) === product.images.length - 1 ? "disabled" : "")
                        }
                        onClick={() => {
                           const imagePos = product.images.indexOf(url)
                           if (imagePos === product.images.length - 1) return
            ...
            <div className="right">
                        ...
                        <div className="discount">
                           <div className="actualPrice">
                              {new Intl.NumberFormat("en-US", {
                                 ...
                                 Math.round(product.price + product.price * (product.discountInPercent / 100))
                              )}
                           </div>
                  ...
                  <div className="cart">
                     ...
                     {cart[product.id] > 0 && (
                        <div className="numbers">
                           <div className="action" onClick={() => Cart.deleteProduct(product.id)}>
                              -
                           </div>
               ...
               <div className="details">
                  ...
                  <ProductInfo heading="Care instructions" content={product.careInstructions} />
                  <ProductInfo heading="Delivery information" content={product.deliveryInfo} />
               </div>
            </div>
         </div>
